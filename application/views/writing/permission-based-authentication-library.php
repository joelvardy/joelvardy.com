<div class="post">
	...
	<p>I can't even count the number of authentication libraries I have written since I started building websites. I would often iterate on the previous library each time I needed it in a project.</p>

	<p>While I was working on several CodeIgniter projects I wrote an authentication library, this served it's purpose however it did not have any permissions, and of course, was limited to being used within CodeIgniter projects.</p>
	<p>I decided to write a new <a href="https://github.com/joelvardy/authentication" title="Open source repository" data-analytics="Authentication Library repo">authentication library</a> which I've released as a Composer package, in this post I'm going to explain how the permission control works.</p>

	<h3>Previously</h3>
	<p>Most of my previous authentication libraries had rudimentary access controls, examples:</p>
...
if ($user_level == 1 || $user_level == 2 || $user_level == 0) {
	...
	<p>Both of these examples will soon result in hard to maintain, messy, hard to read code. It is also easy to accidentally grant permission to the wrong type of user.</p>

	<h3>Much Better</h3>
	<p>Before I started writing any of the library I wrote the code below:</p>
<pre><code class="language-php">if ($auth->permission('moderate-comments')) {
    // This user can moderate comments
}</code></pre>
	<p>As you can see this code is quiet succinct, and the rule doesn't need changing if a new type of user is added, and it is descriptive.</p>

	<h3>The Tables</h3>
	<p>Like most developers, as soon as I had written the three lines above, I was already thinking about the database schema.</p>
	<p>There are three tables, these are:</p>
	<ul>
	...
	<p>In the end I decided to just make it work for me, get a version out there. I can always refactor the library so it uses the Doctrine DBAL at a later point.</p>
